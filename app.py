...
METADATA_FILE = "metadata.json"  # Used by inference.py, contains a list of paths
...
def startup():
    ...
    except (FileNotFoundError, KeyError, json.JSONDecodeError, ValueError, AttributeError):
        print("Index files missing or invalid — rebuild using /build-index")
...
def build_index(img_dir: str, checkpoint: str = "openai"):
    ...
    with _idx_lock:
        ...
        # Post-process the metadata file to ensure paths are relative and use forward slashes.
        # This is a workaround for potential bugs in the underlying inference script.
        try:
            ...
            corrected_paths = []
            for p in paths:
                # Create a relative path from the image path to the indexed directory
                rel_path = os.path.relpath(p, full_img_path)
                # Convert to forward slashes for web compatibility
                corrected_paths.append(rel_path.replace(os.path.sep, '/'))

            g["indexer"].paths = corrected_paths

            with open(METADATA_FILE, "w") as f:
                json.dump(corrected_paths, f)
        except Exception as e:
            print(f"Warning: Could not post-process metadata file. Paths may be incorrect. Error: {e}")
            raise HTTPException(status_code=500, detail="Failed to process metadata after indexing.")
